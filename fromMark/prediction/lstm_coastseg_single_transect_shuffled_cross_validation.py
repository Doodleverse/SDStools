...
def timeseries_to_sequences(data, sequence_length):
    X, y = [], []
...
def setup_data(df,
    ...
    record_length = len(shore_data)
    num_sequences = record_length - look_back
    


    x, y = timeseries_to_sequences(shore_data, look_back)
    ...
    num_train_seqs = int(split_percent*num_sequences)
    num_val_seqs = int((num_sequences-num_train_seqs)/2)
    num_test_seqs = num_sequences - num_train_seqs - num_val_seqs

    x_train = x[:num_train_seqs]
    y_train = y[:num_train_seqs]
    x_train = x_train.reshape((x_train.shape[0], x_train.shape[1], 1))
    y_train = y_train.reshape((y_train.shape[0], y_train.shape[1], 1))

    x_val = x[num_train_seqs:(num_sequences-num_test_seqs)]
    y_val = y[num_train_seqs:(num_sequences-num_test_seqs)]
    x_val = x_val.reshape((x_val.shape[0], x_val.shape[1], 1))
    y_val = y_val.reshape((y_val.shape[0], y_val.shape[1], 1))

    x_test = x[(num_train_seqs+num_val_seqs):]
    y_test = y[(num_train_seqs+num_val_seqs):]
    x_test = x_test.reshape((x_test.shape[0], x_test.shape[1], 1))
    y_test = y_test.reshape((y_test.shape[0], y_test.shape[1], 1))

    prediction_generator = TimeseriesGenerator(shore_data, shore_data, length=look_back, batch_size=1)

    data_dict = {'shore_data':shore_data,
                 'x_train':x_train,
                 'y_train':y_train,
                 'x_val':x_val,
                 'y_val':y_val,
...
def train_model(x_train,
    ...
    Training the LSTM
    Structure is
    Stateful Bidirectional LSTM layer
    Stateful Bidirectional LSTM layer
    Dense Layer
    ...
    early_stopping_callback = EarlyStopping(monitor='val_loss', patience=50, mode='auto', restore_best_weights=True)
...
def predict_data(model, prediction_generator, units, look_back):
    """
    Getting predictions from trained model
    """
    # re-define model
    new_model = model
    prediction = new_model.predict_generator(prediction_generator)
    return prediction
...
def project(df,
    ...
    forecast = predict(num_prediction, new_model)
...
def run(csv_path,
    ...
    for i in range(bootstrap):
        print('trial: '+str(i+1))
        reset_keras()
        model, history = train_model(data_dict['x_train'],
                                     data_dict['y_train'],
                                     data_dict['x_val'],
                                     data_dict['y_val'],
                                     look_back,
        ...
        prediction = predict_data(model,
                                  data_dict['prediction_generator'],
                                  units,
                                  look_back)
        
        forecast, forecast_dates = project(df,
                                           data_dict['shore_data'],
                                           look_back,
                                           num_prediction,
                                           model,
                                           units,
        ...
        predict_array[i,:] = prediction[:,0,0]
    ...
    plt.plot(observed_dates, df['position'], color='k', label='Observed Position')
    ...
    plt.legend()
    plt.minorticks_on()
    plt.xlim(min(observed_dates), max(observed_dates))
    plt.tight_layout()
