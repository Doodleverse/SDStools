...
def single_transect(projection_df_path, transect_id, config_gdf_path, firstX, firstY, switch_dir=False):
...
def multiple_transects(projection_df_path_list,
    ...
    crs_wgs84 = gpd.read_file(config_gdf_path).crs
    ...
    for i in range(len(predicted_df_path_list)):
        ...
        firstX = extracted_df['eastings'][0]
        firstY = extracted_df['northings'][0]
        new_predicted_df_path = single_transect(predicted_df_path, transect_id, config_gdf_path, firstX, firstY, switch_dir=switch_dir)
