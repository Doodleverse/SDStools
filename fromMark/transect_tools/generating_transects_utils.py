...
def break_into_parts(input_shorelines):
    ...
    for i in range(len(gdf)):
        ...
        new_gdf = gpd.GeoDataFrame({'geometry':row.geometry},
                                   index=[0],
                                   crs=crs)
        new_name = os.path.join(new_dir, 'shore'+str(i)+'.shp')
...
def shapefile_to_geojson(my_shape_file, out_dir):
    """
    ...
    new_name = os.path.splitext(os.path.basename(my_shape_file))[0]+'.geojson'
...
def geojson_to_shapefile(my_geojson, out_dir):
    """
...
def batch_geojson_to_shapefile(in_dir, out_dir):
    ...
    for jsn in shapefiles:
        geojson_to_shapefile(jsn, out_dir)       
...
def shorelines_to_transects(input_shorelines_path, output_transects_path):
    """
    ...
    i=0
    for line in lines:
        out_transect = make_transects(line,
                                      50,
                                      1000)
...
##shorelines_to_transects(r'C:\Users\mlundine\OneDrive - DOI\MarkLundine\Data\Alaska\AlaskaRefShoreline_missing\alaskarefshoreline_missing.shp',
##                        r'C:\Users\mlundine\OneDrive - DOI\MarkLundine\Data\Alaska\AlaskaRefShoreline_missing\alaskarefshoreline_missing_transects.shp')
##shorelines_to_transects(r'/path/to/2010s_shorelines_elim2unalakeet.geojson',
##                        r'/path/to/2010s_shorelines_elim2unalakeet_transects.geojson')
